...
class ECE(nn.Module):
    def __init__(self, hidden_dim, rank=None, drop=0.1):
        ...
        # Pre-fusion and normalization: cascade + Hadamard + differential (providing richer manual interaction)
        in_dim = hidden_dim * 4 
        ...
        # Squeeze-Excite: Adaptive recalibration by channel
    ...
    def forward(self, cell_feat, other_drug_feat):
        ...
        z0 = torch.cat([c, d, c * d, c - d], dim=-1)     # [B, 4H]
        z  = self.pre(z0)                                # [B, H]

        z_gate = self.geglu(self.geglu_1(z))             # [B, H]
        z_gate = self.proj_1(self.dropout(z_gate))       # [B, H]

        bil = self.U(c) * self.V(d)                      # [B, r]
        bil = self.Wb(self.dropout(bil))                 # [B, H]

        out = z + z_gate + bil                           
        scale = self.se(out)                             # [B, H], in (0,1)
        out = out * scale
...
class FiLM(nn.Module):
    def __init__(self, in_dim, cond_dim, hidden=None):
        ...
        self.gen = nn.Sequential(
            ...
            nn.Linear(h, 2 * in_dim)  # Generate [gamma, beta]
        )
...
class EnvSyn(nn.Module):
    ...
    def forward(self, data1, data2, sup_labels=None):
        """
        Extracts features by interacting cell line data with the paired drug.
        It uses a gating mechanism (GEGLU) and a bilinear transformation 
        to capture high-order relationships between the environment and the compound.
        """
        # Project MoLFormer embeddings (768) to hidden space
        drug1, drug2 = data1.mol, data2.mol
        d1_b, d2_b = self.mol_proj(drug1), self.mol_proj(drug2)

        # Encode cell line gene expression/features
        cell_vec = data1.cell
        cell_feat = self.cell_encoder(cell_vec)               

        # Cross-Conditioning: Inform Drug 1 about Drug 2 + Cell context, and vice-versa
        cond1 = self.cond_merge(cell_feat, d2_b)  
        cond2 = self.cond_merge(cell_feat, d1_b)  

        # Feature Modulation: Use FiLM to scale/shift drug features based on environment
        d1_bi = self.film_embed(d1_b, cond1)    # A ← (cell, B)
        d2_bi = self.film_embed(d2_b, cond2)    # B ← (cell, A)

        # Dual Interaction: Combine original and environment-modulated interactions
